...
# Auto-refresh real (antes estaba importado pero nunca usado)
...
def limpiar_nombres_columnas(df):
    ...
    for col in df.columns:
        ...
        nuevas_columnas.append(mapeo_nombres.get(col_upper, col_upper))
    df.columns = nuevas_columnas
    return df


def eliminar_filas_encabezado_repetido(df):
    """
    NUEVO: en vez de filtrar valor-por-valor con una lista genérica de palabras
    (lo cual borraba datos válidos), esto detecta filas COMPLETAS que son en
    realidad un encabezado repetido dentro de los datos (por ejemplo cuando el
    Excel tiene sub-encabezados intermedios) y las elimina una sola vez.
    """
    if len(df) == 0:
        return df

    columnas_norm = [str(c).upper().strip() for c in df.columns]

    def es_fila_encabezado(row):
        valores_fila = [str(v).upper().strip() if pd.notna(v) else '' for v in row.values]
        coincidencias = sum(1 for v, c in zip(valores_fila, columnas_norm) if v == c and v != '')
        # Si al menos la mitad de las columnas de esta fila repiten literalmente
        # el nombre de su propia columna, es un encabezado duplicado, no un dato real.
        return coincidencias >= max(2, int(len(columnas_norm) * 0.5))

    mascara = df.apply(es_fila_encabezado, axis=1)
    if mascara.any():
        df = df[~mascara].copy()
    return df


def limpiar_nombre_columna_para_mostrar(col):
    return col


def limpiar_datos_categoricos(df, columna):
    """
    CORREGIDO: antes esta función borraba TODO valor numérico puro
    (ej. códigos de sector "01","02") y todo valor que coincidiera con
    una lista fija de palabras ("TOTAL","TIPO","ESTADO", etc.), lo que
    hacía desaparecer datos reales y válidos.

    Ahora solo se descarta un valor si es EXACTAMENTE igual al nombre
    de esta columna específica (es decir, un encabezado repetido real
    dentro de la data), no contra una lista genérica.
    """
    df_limpio = df.copy()
    valores = df_limpio[columna].copy()

    # 1. Quitar nulos / vacíos
    valores = valores[valores.notna()]
    valores = valores[valores.astype(str).str.strip() != '']

    # 2. Quitar solo si el valor es igual al nombre de ESTA columna (encabezado repetido)
    valores = valores[valores.astype(str).str.upper().str.strip() != columna.upper().strip()]

    # 3. Normalizar texto (tildes, mayúsculas) — ya NO se descarta por ser número
    #    ni por coincidir con una palabra "común" genérica.
    valores_normalizados = valores.apply(normalizar_texto)
    valores_normalizados = valores_normalizados.dropna()

    if len(valores_normalizados) == 0:
        return pd.DataFrame(columns=[columna, 'Cantidad', 'Porcentaje'])

    conteos = valores_normalizados.value_counts().reset_index()
    conteos.columns = [columna, 'Cantidad']
    total = conteos['Cantidad'].sum()
    conteos['Porcentaje'] = (conteos['Cantidad'] / total * 100).round(2) if total > 0 else 0
    return conteos
...
def generar_grafico_numerico(df, columna):
    ...
    fig_hist.update_layout(
        ...
        xaxis_title=columna, yaxis_title='Frecuencia', showlegend=False,
        xaxis=dict(showgrid=True, gridcolor='#e2e8f0', tickfont=dict(color='#000000', size=12)),
        yaxis=dict(showgrid=True, gridcolor='#e2e8f0', tickfont=dict(color='#000000', size=12))
    )
    figs['histograma'] = fig_hist

    fig_box = px.box(df_num, y=columna,
                    title=f'📦 DISTRIBUCIÓN ESTADÍSTICA: {columna}',
                    color_discrete_sequence=['#f5576c'], points='all')
    fig_box.update_layout(
        height=450, plot_bgcolor='white', paper_bgcolor='white',
        font=dict(size=13, color='#000000'), title_font=dict(size=18, color='#000000'),
        yaxis_title=columna,
        yaxis=dict(showgrid=True, gridcolor='#e2e8f0', tickfont=dict(color='#000000', size=12))
    )
    figs['boxplot'] = fig_box
    figs['estadisticas'] = df_num[columna].describe()
    return figs
...
def generar_grafico_categorico(df, columna):
    ...
    fig_bar = px.bar(conteos, y=columna, x='Cantidad',
                    ...
                    text='Cantidad', orientation='h')
    fig_bar.update_traces(textposition='outside', marker_line_color='#000000', marker_line_width=1)
    fig_bar.update_layout(
        height=max(400, len(conteos) * 35), plot_bgcolor='white', paper_bgcolor='white',
        font=dict(size=12, color='#000000'), title_font=dict(size=18, color='#000000'),
        xaxis_title='Cantidad', yaxis_title=columna, showlegend=False,
        margin=dict(l=200, r=50, t=80, b=50),
        xaxis=dict(showgrid=True, gridcolor='#e2e8f0', tickfont=dict(color='#000000', size=12)),
        yaxis=dict(showgrid=False, tickfont=dict(color='#000000', size=12, family='Arial'))
    )
    figs['barras'] = fig_bar
    ...
    if len(conteos) <= 15:
        fig_pie = px.pie(conteos, values='Cantidad', names=columna,
                        ...
                        hole=0.4, color_discrete_sequence=PALETA_PRINCIPAL)
...
def generar_grafico_temporal(df, columna):
    ...
    fig_line = px.line(evol, x='Año-Mes', y='Cantidad',
                      title=f'📈 EVOLUCIÓN TEMPORAL: {columna}', markers=True)
    fig_line.update_traces(line=dict(color='#667eea', width=3), marker=dict(size=8, color='#f5576c'))
    fig_line.update_layout(
        height=450, plot_bgcolor='white', paper_bgcolor='white',
        font=dict(size=13, color='#000000'), title_font=dict(size=18, color='#000000'),
        xaxis_title='Período', yaxis_title='Cantidad',
        xaxis=dict(tickangle=-45, showgrid=True, gridcolor='#e2e8f0', tickfont=dict(color='#000000', size=12)),
        yaxis=dict(showgrid=True, gridcolor='#e2e8f0', tickfont=dict(color='#000000', size=12))
    )
    ...
    fig_bar = px.bar(por_año, x='Año', y='Cantidad',
                    ...
                    color='Cantidad', color_continuous_scale='Plasma', text='Cantidad')
...
def eliminar_columnas_duplicadas(df):
    ...
    for col in df.columns:
        ...
        if col_str in columnas_vistas:
            ...
            nuevas_columnas.append(f"{col_str}_{columnas_vistas[col_str]}")
        else:
...
@st.cache_data(ttl=300)  # antes 30s: demasiado frecuente para un archivo pesado
def cargar_datos():
    try:
        response = requests.get(URL_EXCEL, timeout=30)

        if response.status_code != 200:
            st.error(f"Error HTTP {response.status_code} al descargar el archivo.")
            return pd.DataFrame()
        ...
        es_excel_valido = (
            ...
            or response.content[:2] == b'PK'  # firma binaria de archivos .xlsx
        )
        if not es_excel_valido:
            st.error(
                "El link de SharePoint no devolvió un Excel válido "
                f"(Content-Type recibido: '{content_type}'). "
                "Verifica que el enlace tenga acceso anónimo/público habilitado, "
                "o que no haya expirado la sesión de 'compartir'."
            )
            return pd.DataFrame()
        ...
        df = eliminar_filas_encabezado_repetido(df)  # NUEVO: reemplaza el filtrado agresivo por valor
...
def inicializar_analisis_columnas():
    ...
    for col in df.columns:
        try:
            ...
            analisis[col] = {'tipo': 'texto', 'unicos': 0, 'nulos': 0, 'total': len(df)}
    return analisis
...
with st.sidebar:
    ...
    if st.button("🔄 Forzar recarga de datos", use_container_width=True, key="reload"):
        st.cache_data.clear()
        st.rerun()
...
if st.session_state.pagina_actual == 'inicio':
    ...
    with col1:
        st.markdown(f"""<div class="kpi-card"><p class="kpi-value">{len(df)}</p>
        <p class="kpi-label">Total Registros</p></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="kpi-card"><p class="kpi-value">{len(df.columns)}</p>
        <p class="kpi-label">Columnas</p></div>""", unsafe_allow_html=True)
    with col3:
        categoricas = sum(1 for v in st.session_state.analisis_columnas.values() if v.get('tipo') == 'categorica')
        st.markdown(f"""<div class="kpi-card"><p class="kpi-value">{categoricas}</p>
        <p class="kpi-label">Variables Categóricas</p></div>""", unsafe_allow_html=True)
    with col4:
        numericas = sum(1 for v in st.session_state.analisis_columnas.values() if v.get('tipo') == 'numerica')
        st.markdown(f"""<div class="kpi-card"><p class="kpi-value">{numericas}</p>
        <p class="kpi-label">Variables Numéricas</p></div>""", unsafe_allow_html=True)
...
# ==========================================
    ...
    if col_estado and col_año:
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig = px.bar(df_tasas, x='Año', y='Tasa (%)', title='TASA DE CULMINACIÓN (%)',
                        color='Tasa (%)', color_continuous_scale='RdYlGn', text='Tasa (%)')
            fig.update_traces(textposition='outside')
            fig.update_layout(yaxis_range=[0, 100], height=450, plot_bgcolor='white', paper_bgcolor='white',
                            font=dict(size=13, color='#000000'), title_font=dict(size=18, color='#000000'))
    ...
    if col_sector:
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig = px.pie(sector_stats, values='Cantidad', names='Sector', title='DISTRIBUCIÓN POR SECTOR',
                        hole=0.4, color_discrete_sequence=PALETA_PRINCIPAL)
...
# ==========================================
# FOOTER (antes mostraba literalmente "{fecha}" en pantalla)
# ==========================================
st.markdown(f"""
<div class="footer">
